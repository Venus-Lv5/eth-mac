class int_rx_pause_seen_100M_test extends eth_base_test;
  `uvm_component_utils(int_rx_pause_seen_100M_test)

  function new(string name="int_rx_pause_seen_100M_test", uvm_component parent);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    wdata_q.push_back(32'h0000_0002);                 // RX PAUSE enabled
    ...
    pause_seq = phy_pause_frame_seq::type_id::create("rx_pause_seen_seq");
    assert(pause_seq.randomize() with {
      dst_mac == 48'h01_80_C2_00_00_01;
      src_mac == cfg.phy_addr;
      pause_timer == 16'h0001;
    })
    else `uvm_fatal(get_type_name(), "Failed to randomize RX_PAUSE_SEEN sequence")
    pause_seq.start(env.phy_agt.sequencer);

    wait_interrupt_or_timeout(got_irq);
    regmodel.FSR.read(status, rdata);
    if (!got_irq)
      `uvm_error(get_type_name(), "RX_PAUSE_SEEN interrupt timeout")
    if (!rdata[9])
      `uvm_error(get_type_name(), $sformatf("RX_PAUSE_SEEN status mismatch: FSR=0x%08h", rdata))
    if (got_irq && rdata[9])
      `uvm_info(get_type_name(), $sformatf("INT: RX_PAUSE_SEEN IRQ_SEEN=%0b FSR=0x%08h", got_irq, rdata), UVM_NONE)
